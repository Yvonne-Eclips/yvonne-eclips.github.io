document.addEventListener('DOMContentLoaded', function() {
    ...
    // This is the most reliable way to ensure only specific videos appear here.
    ...
    const ITEMS_PER_API_CALL = 50; // Max results per page for YouTube API (for efficiency)
     ...
     * Tries fetching via `videoDuration=short` first, then falls back to aspect ratio filter.
    ...
    async function fetchLatestShorts(containerElement) {
        ...
        // Common Shorts are 9:16 (0.5625) or 3:4 (0.75). 0.8 allows slight variations.
        const VERTICAL_ASPECT_RATIO_THRESHOLD = 0.8; 

        try {
            containerElement.innerHTML = '<p class="loading-message">Loading latest Shorts...</p>';

            // --- Attempt 1: Fetch using videoDuration=short API filter ---
            ...
            if (data1.items && data1.items.length > 0) {
                shortsToDisplay = shortsToDisplay.concat(data1.items);
            }

            // --- Attempt 2 (Fallback/Refinement): Filter by aspect ratio if needed ---
            // If we don't have enough from the direct 'short' filter, fetch more general videos
            // and apply aspect ratio filtering for a more robust detection.
            while (shortsToDisplay.length < maxResults && data1.nextPageToken) {
                 nextPageToken = data1.nextPageToken; // Use next page token from previous call
                 searchUrl = `https://www.googleapis.com/youtube/v3/search?part=id,snippet&channelId=${CHANNEL_ID}&type=video&order=date&key=${API_KEY}&maxResults=${ITEMS_PER_API_CALL}&pageToken=${nextPageToken}`;
                 const response2 = await fetch(searchUrl);
                 const data2 = await response2.json();

                 if (!data2.items || data2.items.length === 0) break; // No more videos

                 const videoIds = data2.items.map(item => item.id.videoId).filter(id => id);
                 if (videoIds.length === 0) continue;

                 // Fetch details to get thumbnail dimensions for aspect ratio check
                 ...
                 data1.nextPageToken = data2.nextPageToken; // Update token for next loop
            }
            ...
            if (shortsToDisplay.length > 0) {
                ...
                shortsToDisplay.slice(0, maxResults).forEach(item => {
                    // Item.id is typically available directly from 'videos' API endpoint,
                    // otherwise item.id.videoId from 'search' endpoint. Handle both for robustness.
     ...
     * This function is explicitly used for the 'Latest Videos' section to display curated content.
     * It relies entirely on the content of the provided playlist ID.
        ...
        try {
            ...
            if (data.items && data.items.length > 0) {
                data.items.forEach(item => {
                    const videoId = item.snippet.resourceId.videoId; // For playlistItems, video ID is here
                    const videoTitle = item.snippet.title;
                    appendVideoIframe(containerElement, videoId, videoTitle);
                });
            } else {
    ...
    async function fetchPopularVideos(containerElement) {
        try {
            ...
            if (data.items && data.items.length > 0) {
                data.items.forEach(item => {
                    const videoId = item.id.videoId; // For search endpoint, video ID is here
                    const videoTitle = item.snippet.title;
